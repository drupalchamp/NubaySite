<link href='https://fonts.googleapis.com/css?family=Open+Sans:400,300,600,600italic,700,700italic,800,800italic|Open+Sans+Condensed:300,700' rel='stylesheet' type='text/css'>
...
//theme setting for node/add/forum/* page
  if(arg(0) == 'node' && arg(1) == 'add' && arg(2) == 'forum' && is_numeric(arg(3))){
    $url_tid = arg(3);
	$values = taxonomy_get_parents($url_tid);
	foreach($values as $value){ 
	}
	if($value->tid=="7" || $val=="7"){
	  $themename = "wbf";
  ...
  //theme setting for forum content type page
  //theme setting for wbf slideshow page
  if(arg(0) == 'node' && is_numeric(arg(1))){
    $nid = arg(1);
    $node_obj = node_load($nid);
    if($node_obj->type == 'forum'){
      $tax_value = $node_obj->taxonomy_forums['und'][0]['tid'];
      $values = taxonomy_get_parents($tax_value);
	  foreach($values as $value){ 
	  }
	  if($value->tid=="7" || $val=="7"){
	    $themename = "wbf";
	 }
   }elseif($node_obj->type == 'wbf_slideshow'){
     $themename = "wbf";
   }
 }
 
 //theme setting for group/node/*/subscribe/og_user_node page
  if(arg(0) == 'group' && arg(1) == 'node' && is_numeric(arg(2)) && arg(3) == 'subscribe' && arg(4) == 'og_user_node'){
    $nid = arg(2);
    $node_obj = node_load($nid);
	if($node_obj->field_select_layout['und'][0]['value'] == 'wbf'){
	  $themename = "wbf";
	}
  }
  
   //theme setting for group/node/*/unsubscribe page
  if(arg(0) == 'group' && arg(1) == 'node' && is_numeric(arg(2)) && arg(3) == 'unsubscribe'){
    $nid = arg(2);
    $node_obj = node_load($nid);
	if($node_obj->field_select_layout['und'][0]['value'] == 'wbf'){
	  $themename = "wbf";
  ...
  if(strpos(@$_SERVER['REDIRECT_URL'], 'wbf')) {
    if( arg(0) == 'civicrm'){
      $themename = "wbf";
    }
  }
  
  /*if(arg(0) == 'wbf' || arg(0) == 'wbf-event-photos'){
	  $themename = "wbf";
  }*/
